...
(function (scope)
{
	...
	function isDate(o)
	{
		return '[object Date]' === _toString.call(o);
	}

	function isRegExp(o)
	{
		return '[object RegExp]' === _toString.call(o);
	}
...
if (document.location.hostname === 'sellfy.com')
{
...
(function ($)
{
	var sellfy = this.sellfy || {};
	sellfy.helpers = {
		addUrlGetParameter: function (url, parameter)
		{
			if (url && typeof url === 'string')
			{
				var seperator = (url.indexOf('?') === -1) ? '?' : '&', clean_parameter = encodeURI(seperator + parameter);
				url += clean_parameter;
				return url;
			}
			else
			{
				throw'sellfy.helpers.addUrlGetParameter: `url` argument should be string';
			}
		}, redirect: function (url, replace_in_history)
		{
		...
		}, keys: {backspace: 8, tab: 9, enter: 13, space: 32, comma: 188}, show_alert: function (header, text)
		{
			var template = '<div class="ui modal"><i class="close icon"></i> <div class="header">' + header + '</div> <div class="content">' + text + '</div> <div class="actions"> <div class="ui button green">Ok</div> </div> </div>';
			$(template).modal('show');
		}, pluralize: function (number, singular, plural)
		{
			singular = typeof singular !== 'undefined' ? singular : '';
			plural = typeof plural !== 'undefined' ? plural : 's';
...
if (!String.prototype.truncate)
{
	...
	{
		var length = arguments[0] || 255, elipsis = arguments[1] || '...';
		if (this.length > length)
		{
			return this.substring(0, length) + elipsis.toString();
		}
		else
		{
			return this;
...
$(function ()
{
	var $sidebar = $('.header-sidebar'), $sidebarToggle = $('.header-sidebar-toggle'), $sidebarClose = $('.header-sidebar-close'), $body = $('body'), showSidebar = function ()
	{
	...
	}, hideSidebar = function ()
	{
		...
		$sidebar.addClass('animating').removeClass('visible');
	};
	$sidebarToggle.on('click', showSidebar);
	$sidebar.find('.item').on('click', hideSidebar);
	$sidebarClose.on('click', hideSidebar);
	$('#cookie_consent').on('click', function ()
	{
		$(this).parents('.cookie-consent').remove();
		var year = 31536000, options = {path: '/', maxAge: year * 5,};
		Cookie.set('cookie-consent', true, options);
	});
});
!function (a)
{
...
(function ($)
{
	sellfy.CountdownView = function (selector)
	{
		var $counters = $(selector), _this = this;
		...
		this.start = function ()
		{
			...
			{
				var $this = $(this), finalDate = new Date($this.data('end'));
				$this.countdown(finalDate, _this.updateTimer);
			});
		};
	};
	this.sellfy = sellfy;
...
$(function ()
{
	var $featureList = $('#feature_list'), $featureItems = $featureList.find('.item'), $backgroundItem = $featureList.find('.background-item'), $productSelector = $('#product_category_select'), $productSelectItem = $productSelector.find('.item'), $productHolder = $('#product_holder');
	...
	{
		var $this = $(this), className = $this.data('class'), height = $this.outerHeight();
		$backgroundItem.height(height);
		$featureList.removeClass().addClass(className);
	});
	$productSelectItem.on('click', function ()
	{
		var $this = $(this), className = $this.data('class');
		$productSelectItem.removeClass('active');
		$this.addClass('active');
		$productHolder.removeClass().addClass(className);
	});
	var countdown = new sellfy.CountdownView('.bundle-end-countdown');
	countdown.start();
});